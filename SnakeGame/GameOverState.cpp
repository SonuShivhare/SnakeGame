...
void GameOverState::init()
{
	...
	homeButton.setPosition(window_Width / 10, window_Height / 1.2);
	...
	quitButton.setPosition(window_Width / 1.11, window_Height / 1.2);
	...
	currentScore.setString(std::to_string(highScore.returnPlayerScore()));
	currentScore.setPosition(sf::Vector2f(window_Width / 2, window_Height / 3.5));
	currentScore.setOrigin(sf::Vector2f(currentScore.getGlobalBounds().width / 2, currentScore.getGlobalBounds().height / 2));
	
	highestScore.setFont(this->data->assets.getFont("font"));
	highestScore.setFillColor(sf::Color::Black);
	highestScore.Bold;
	highestScore.setCharacterSize(45);
	highestScore.setString(std::to_string(highScore.returnHighestScore()));
	highestScore.setPosition(sf::Vector2f(window_Width / 2, window_Height / 1.6));
